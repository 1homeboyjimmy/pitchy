...
export default function ResidentAcceleratorPage() {
  ...
  return <main className="min-h-[100dvh] bg-black px-4 py-7 text-white sm:px-8 sm:py-10"><div className="mx-auto max-w-7xl">
    <header className="mb-7 flex flex-wrap items-center justify-between gap-4"><div><Link href={memberships.length > 1 ? "/accelerator" : "/dashboard"} className="inline-flex items-center gap-2 text-sm text-white/40 hover:text-white"><ArrowLeft size={15} /> {memberships.length > 1 ? "Все акселераторы" : "В дашборд"}</Link><div className="mt-4 flex items-center gap-3"><Rocket className="text-white/45" /><h1 className="text-3xl tracking-tight sm:text-5xl">Моё участие</h1></div></div><div className="flex flex-wrap items-center justify-end gap-2">{memberships.length > 1 && <label className="min-w-56 text-xs uppercase tracking-wide text-white/35">Поток<select value={numericMembershipId} onChange={(event) => router.push(`/accelerator/my/${event.target.value}`)} className="workspace-input mt-2 normal-case tracking-normal text-white">{memberships.map((row) => <option key={row.membership_id} value={row.membership_id}>{row.accelerator.name} · {row.cohort.name}</option>)}</select></label>}{serviceAccelerator && <Link href={`/accelerator?context=staff&accelerator=${serviceAccelerator.id}`} className="workspace-button"><BriefcaseBusiness size={15} /> Служебный кабинет</Link>}<NotificationCenter token={token} /></div></header>
