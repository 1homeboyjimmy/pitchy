...
import { Archive, Bell, Clock3, Copy, Loader2, Pencil, Plus, Send, Sparkles, Users, X } from "lucide-react";
...
const emptyForm = (pitchyEnabled: boolean) => ({ title: "", description: "", dueAt: "", stageId: "", audience: "cohort" as "cohort" | "selected", targetIds: [] as number[], allowResubmit: true, assignmentType: "text_files" as "text_files" | "quiz", submissionMode: "individual" as "individual" | "team", passingScore: 70, maxAttempts: 1, quizQuestions: [] as QuizQuestion[], pitchyEnabled });
...
export function HomeworkManager({ cohortId, token, residents, isAdmin, pitchyEnabled, focusId }: { cohortId: number; token: string; residents: Resident[]; isAdmin: boolean; pitchyEnabled: boolean; focusId?: number }) {
  const [assignments, setAssignments] = useState<Assignment[]>([]);
  useDashboardFocus("homework", focusId, assignments.length);
  const [stages, setStages] = useState<ProgramStage[]>([]);
  const [form, setForm] = useState(() => emptyForm(pitchyEnabled));
  const [editingId, setEditingId] = useState<number | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [formTab, setFormTab] = useState<"main" | "execution" | "settings">("main");
  ...
  useEffect(() => { const close = (event: KeyboardEvent) => { if (event.key === "Escape") { setEditingId(null); setShowForm(false); } }; window.addEventListener("keydown", close); return () => window.removeEventListener("keydown", close); }, []);
  ...
  const resetForm = () => { setForm(emptyForm(featureEnabled)); setEditingId(null); setShowForm(false); setFormTab("main"); };
  const edit = (assignment: Assignment) => {
    setForm({
      ...
      pitchyEnabled: assignment.pitchy_enabled,
    });
    setEditingId(assignment.id); setFormTab("main"); setShowForm(true);
  };
  ...
  const save = async (event: FormEvent) => {
    ...
    const payload = {
      ...
      pitchy_enabled: form.pitchyEnabled,
    };
    try {
  ...
  return (
    <section className="workspace-card">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl">Все задания</h2><p className="mt-1 text-sm text-white/40">Создавайте задания, тесты и задавайте аудиторию.</p>{isAdmin && <p className="mt-1 text-xs text-white/30">Общая настройка Pitchy используется только как значение по умолчанию для новых заданий.</p>}</div><div className="flex flex-wrap gap-2">{isAdmin && <button type="button" onClick={() => void togglePitchy()} disabled={busy === "pitchy-feature"} className="overview-secondary"><Sparkles size={15} />Pitchy по умолчанию: {featureEnabled ? "включён" : "выключен"}</button>}<button onClick={() => { if (showForm) resetForm(); else { setForm(emptyForm(featureEnabled)); setFormTab("main"); setShowForm(true); } }} className="workspace-button"><Plus size={15} />{showForm ? "Закрыть" : "Новое задание"}</button></div></div>
      ...
      {showForm && <div className="fixed inset-0 z-[80] flex justify-end bg-black/65" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) resetForm(); }}><form onSubmit={save} role="dialog" aria-modal="true" aria-labelledby="homework-form-title" className="h-full w-full max-w-3xl overflow-y-auto border-l border-white/10 bg-[#1c1b1b] p-5 shadow-2xl sm:p-6"><div className="mb-5 flex items-start justify-between"><div><div className="flex flex-wrap items-center gap-3"><h3 id="homework-form-title" className="text-2xl">{editingId ? "Редактирование задания" : "Новое задание"}</h3><span className="text-sm text-violet-300">Черновик</span></div><p className="mt-1 text-sm text-white/40">{form.title || "Основное, выполнение и настройки"}</p></div><button type="button" onClick={resetForm} aria-label="Закрыть" className="rounded-full p-2 text-white/45 hover:bg-white/5"><X /></button></div><div className="mb-5 grid grid-cols-3 border-b border-white/10">{([["main","Основное","homework-main"],["execution","Выполнение","homework-execution"],["settings","Настройки","homework-settings"]] as const).map(([key,label,target])=><button key={key} type="button" onClick={()=>{setFormTab(key);document.getElementById(target)?.scrollIntoView({behavior:"smooth",block:"start"})}} className={`border-b-2 px-2 py-3 text-sm ${formTab===key?"border-white text-white":"border-transparent text-white/45"}`}>{label}</button>)}</div><div id="homework-main" className="grid scroll-mt-24 gap-4 sm:grid-cols-2"><label className="text-sm text-white/60">Название<input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} minLength={2} maxLength={300} required className="workspace-input mt-2" /></label><label className="text-sm text-white/60">Дедлайн<input type="datetime-local" value={form.dueAt} onChange={(event) => setForm({ ...form, dueAt: event.target.value })} className="workspace-input mt-2" /></label><label id="homework-execution" className="scroll-mt-24 text-sm text-white/60">Тип ответа<select value={form.assignmentType} onChange={(event) => setForm({ ...form, assignmentType: event.target.value as "text_files" | "quiz", allowResubmit: event.target.value === "quiz" ? true : form.allowResubmit })} className="workspace-input mt-2"><option value="text_files">Текст или файлы</option><option value="quiz">Тест</option></select></label><label className="text-sm text-white/60">Кто выполняет<select value={form.submissionMode} onChange={(event) => setForm({ ...form, submissionMode: event.target.value as "individual" | "team" })} className="workspace-input mt-2"><option value="individual">Каждый участник</option><option value="team">Один ответ от команды</option></select></label><label className="text-sm text-white/60 sm:col-span-2">Описание и ожидаемый результат<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={6} required className="workspace-input mt-2 resize-y" /></label><label className="text-sm text-white/60">Этап программы<select value={form.stageId} onChange={(event) => setForm({ ...form, stageId: event.target.value })} className="workspace-input mt-2"><option value="">Без привязки</option>{stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.title}</option>)}</select></label><label className="text-sm text-white/60">Кому выдать<select value={form.audience} onChange={(event) => setForm({ ...form, audience: event.target.value as "cohort" | "selected", targetIds: [] })} className="workspace-input mt-2"><option value="cohort">Всему потоку</option><option value="selected">Выбранным резидентам</option></select></label>{form.assignmentType === "quiz" ? <><label className="text-sm text-white/60">Проходной балл, %<input type="number" min={0} max={100} value={form.passingScore} onChange={(event) => setForm({ ...form, passingScore: Number(event.target.value) })} className="workspace-input mt-2" /></label><label className="text-sm text-white/60">Количество попыток<input type="number" min={1} max={20} value={form.maxAttempts} onChange={(event) => setForm({ ...form, maxAttempts: Number(event.target.value) })} className="workspace-input mt-2" /></label></> : <label className="flex items-end gap-3 pb-3 text-sm text-white/60"><input type="checkbox" checked={form.allowResubmit} onChange={(event) => setForm({ ...form, allowResubmit: event.target.checked })} /> Разрешить повторную отправку</label>}<label id="homework-settings" className="flex scroll-mt-24 items-end gap-3 pb-3 text-sm text-white/60 sm:col-span-2"><input type="checkbox" checked={form.pitchyEnabled} onChange={(event) => setForm({ ...form, pitchyEnabled: event.target.checked })} /> Разрешить инструменты Pitchy именно в этом задании</label></div>
        ...
        <div className="sticky bottom-0 mt-6 flex justify-end gap-2 border-t border-white/8 bg-[#1c1b1b] py-4"><button type="button" onClick={resetForm} className="overview-secondary">Отмена</button><button disabled={busy === "save"} className="workspace-button">{busy === "save" && <Loader2 size={15} className="animate-spin" />} Сохранить черновик</button></div></form></div>}
