...
export function AttendanceManager({ cohortId, token }: { cohortId: number; token: string }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      const payload = { title: form.title, description: form.description || null, starts_at: new Date(form.startsAt).toISOString(), ends_at: new Date(form.endsAt).toISOString(), event_format: form.format, location: form.location || null, meeting_url: form.meetingUrl || null };
  ...
  const edit = (event: EventRow) => {
    ...
    setForm({ title: event.title, description: event.description || "", startsAt: local(event.starts_at), endsAt: local(event.ends_at), format: event.event_format, location: event.location || "", meetingUrl: event.meeting_url || "" });
