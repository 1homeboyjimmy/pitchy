...
const bundleBrands: { name: string; serif: boolean; role: string; points: string[] }[] = [
  {
    name: "Pitchy",
    serif: false,
    role: "Ведёт проект от идеи до результата",
    points: [
      "Паспорт проекта и командный доступ",
      "Проверка спроса, CustDev и полная аналитика проекта",
      "Дорожная карта, подача на гранты и генерация презентаций проекта",
      "Меньше ручной работы и операционных расходов организатора",
    ],
  },
  {
    name: "Вайбли",
    ...
    points: [
      "AI-разработчик сайтов и игр. Готовый прототип — за часы",
      "Живая ссылка на проект — демонстрация проекта прямо во время питча",
      "Навык кода — не нужен. Команда собирает прототип промптами",
      "Серверы в России, работа без VPN, все RU-интеграции включены",
    ],
  },
];

const bundleFlow: { step: string; by: string | null; desc: string }[] = [
  { step: "Паспорт", by: "Pitchy", desc: "Идея, аудитория, гипотеза, основная информация о проекте" },
...
const benefits: { icon: LucideIcon; title: string; desc: string }[] = [
  ...
  },
  {
    ...
    desc: "Формы и таблицы заменяются единым цифровым решением потока.",
  },
  {
    icon: Rocket,
    title: "Быстрый запуск потока",
    desc: "Поток разворачивается в кратчайшие сроки — без интеграционного ада.",
  },
];
