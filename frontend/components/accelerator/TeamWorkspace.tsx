...
export type TeamInvitation = {
  ...
  invitee?: { membership_id: number; name: string } | null;
...
export function TeamWorkspace({ membershipId, project }: { membershipId: number; project?: { id: number; name: string } | null }) {
  ...
  const [mode, setMode] = useState<"find" | "create" | "applications">("create");
  ...
  const inviteCandidate = async (candidate: PoolContact & { membership_id: number }) => {
    if (!token || !team?.can_manage) return; setBusy(`invite-${candidate.membership_id}`); setError("");
    try {
      await postAuthJson(`/api/accelerators/teams/${team.id}/invitations`, {
        counterpart_profile_id: candidate.profile_id || undefined,
        membership_id: candidate.membership_id,
        message: invitationMessages[candidate.membership_id] || null,
      }, token);
      await load();
  ...
  const invitedMembershipIds = useMemo(() => new Set(pendingTeamInvitations.map((row) => row.invitee?.membership_id).filter((id): id is number => Boolean(id))), [pendingTeamInvitations]);
  ...
  return <div className="space-y-5">
    <section>
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-semibold sm:text-3xl">{team?.name || "Команда"}</h1><p className="mt-1 text-sm text-white/45">{team ? "Участники, роли и поддержка проекта" : "Найдите подходящую команду или соберите свою"}</p></div><div className="flex items-center gap-3">{!team && <span className="rounded-full border border-white/10 px-4 py-2 text-sm text-white/65">Без команды</span>}<button type="button" onClick={() => void load()} disabled={Boolean(busy)} className="rounded-full border border-white/10 p-3 text-white/45" aria-label="Обновить команду"><RefreshCw size={16} /></button></div></div>

      {team && !team.can_manage && !viewerActive && <p className="mt-4 rounded-2xl border border-white/8 bg-white/[.025] p-4 text-sm text-white/50">Вы больше не состоите в этой команде. История состава доступна только для просмотра.</p>}
      ...
      {!team && <div className="mt-6 flex overflow-x-auto border-b border-white/10"><Mode active={mode === "find"} onClick={() => setMode("find")}>Найти команду</Mode><Mode active={mode === "create"} onClick={() => setMode("create")}>Создать свою</Mode><Mode active={mode === "applications"} onClick={() => setMode("applications")}>Мои заявки <span className="ml-1 rounded-md bg-white/10 px-2 py-0.5 text-xs">{pool.applications.filter(row => row.can_cancel).length}</span></Mode></div>}
      {!team && mode === "create" && <form onSubmit={createTeam} className="mt-6 overflow-hidden rounded-2xl border border-white/10"><div className="grid lg:grid-cols-[1fr_360px]"><div className="p-5 sm:p-6"><h3 className="text-xl">Создать команду</h3><p className="mt-1 text-sm text-white/40">Соберите участников вокруг своего проекта</p><div className="mt-5 rounded-2xl border border-white/9 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-[.15em] text-white/35">Проект</p><p className="mt-2 text-lg">{project?.name || "Проект будет добавлен позже"}</p><p className="mt-1 text-sm text-white/40">После создания вы станете капитаном команды</p></div><span className="rounded-full bg-emerald-400/10 px-3 py-1.5 text-xs text-emerald-300">Будет привязан</span></div></div><div className="mt-5 grid gap-4 sm:grid-cols-[1fr_200px]"><label className="text-sm text-white/55">Название команды<input aria-label="Название" value={createForm.name} onChange={(event) => setCreateForm({ ...createForm, name: event.target.value })} minLength={2} maxLength={200} required className="workspace-input mt-2" /></label><label className="text-sm text-white/55">Максимум участников<input type="number" min={2} max={20} value={createForm.maxMembers} onChange={(event) => setCreateForm({ ...createForm, maxMembers: Number(event.target.value) })} className="workspace-input mt-2" /></label></div><button aria-label="Создать" disabled={busy === "create"} className="workspace-button mt-5"><Users size={15} /> Создать команду</button></div><aside className="border-t border-white/8 bg-white/[.018] p-5 sm:p-6 lg:border-l lg:border-t-0"><h3 className="text-lg">После создания</h3><div className="mt-5 space-y-5">{["Приглашайте участников потока","Управляйте ролями команды","Получите общего трекера"].map(item=><p key={item} className="flex items-center gap-3 text-sm text-white/65"><span className="grid size-8 place-items-center rounded-full bg-emerald-400/15 text-emerald-300"><Check size={15}/></span>{item}</p>)}</div></aside></div></form>}
      {!team && mode === "find" && <div className="mt-6 pt-2"><h3 className="text-lg">Открытые команды</h3><p className="mt-1 text-sm text-white/40">Выберите команду, раскройте описание проекта и отправьте заявку.</p><div className="mt-4 grid gap-3 lg:grid-cols-2">{pool.teams.filter((row) => row.recruiting_open && row.member_count < row.max_members).map((row) => <article key={row.id} className="rounded-2xl border border-white/9 p-4"><div className="flex items-start justify-between gap-3"><div><p>{row.name}</p><p className="mt-1 text-xs text-white/35">Капитан: {row.captain.name} · {row.member_count}/{row.max_members}</p></div></div><p className="mt-3 line-clamp-3 text-sm text-white/50">{row.summary}</p><details className="mt-3 rounded-xl border border-white/7 p-3"><summary className="cursor-pointer text-xs text-white/50">Подробнее о проекте</summary><div className="mt-3 space-y-3">{row.details.map((item) => <div key={item.label}><p className="text-xs text-white/30">{item.label}</p><p className="mt-1 whitespace-pre-wrap text-sm text-white/60">{item.value}</p></div>)}<p className="text-xs text-white/35">Компетенции капитана: {row.captain.competencies.join(" · ") || "не указаны"}</p><div className="flex flex-wrap gap-3 text-xs"><a href={`mailto:${row.captain.email}`} className="text-blue-300">{row.captain.email}</a>{row.captain.telegram && <a href={`https://t.me/${row.captain.telegram.replace(/^@/, "")}`} target="_blank" rel="noreferrer" className="text-blue-300">{row.captain.telegram}</a>}</div></div></details><textarea value={messages[row.id] || ""} onChange={(event) => setMessages((current) => ({ ...current, [row.id]: event.target.value }))} rows={2} maxLength={1000} placeholder="Почему хотите присоединиться" className="workspace-input mt-4 resize-y" /><button type="button" onClick={() => void applyToTeam(row.id)} disabled={Boolean(busy) || pendingTeamIds.has(row.id)} className="workspace-button mt-3"><Send size={14} />{pendingTeamIds.has(row.id) ? "Заявка отправлена" : "Подать заявку"}</button></article>)}{!pool.teams.some((row) => row.recruiting_open && row.member_count < row.max_members) && <p className="text-sm text-white/35">Открытых команд пока нет. Если у вас есть проект, создайте свою.</p>}</div></div>}

      {!team && mode === "applications" && <div className="mt-6 overflow-hidden rounded-2xl border border-white/9">{pool.applications.filter(row => row.can_cancel).map(row => <article key={row.id} className="flex flex-wrap items-center justify-between gap-4 border-b border-white/8 p-4 last:border-0"><div><p>{row.team_name}</p><p className="mt-1 text-sm text-white/40">{row.desired_role ? `Желаемая роль: ${row.desired_role}` : "Заявка на вступление"}</p><p className="mt-1 text-xs text-amber-200">Ожидает ответа</p></div><button type="button" onClick={() => void respondApplication(row.id, "cancelled")} disabled={Boolean(busy)} className="overview-secondary">Отозвать заявку</button></article>)}{!pool.applications.some(row => row.can_cancel) && <p className="py-12 text-center text-sm text-white/35">Активных заявок нет.</p>}</div>}      {team && <div className="mt-6">
    ...
    {team?.can_manage && team.status === "active" && <section className="workspace-card"><h2 className="flex items-center gap-2 text-xl"><UserPlus size={18} /> Участники без команды</h2><p className="mt-1 text-sm text-white/40">Откройте карточку участника, посмотрите его опыт и запрос, затем пригласите в свой проект.</p><div className="mt-5 grid gap-3 lg:grid-cols-2">{pool.candidates.map((candidate) => <article key={candidate.membership_id} className="rounded-2xl border border-white/9 p-4"><p>{candidate.name}</p><p className="mt-2 text-sm text-white/50">{candidate.bio || candidate.competencies.join(" · ") || "Описание пока не заполнено"}</p><details className="mt-3 rounded-xl border border-white/7 p-3"><summary className="cursor-pointer text-xs text-white/50">Подробнее об участнике</summary><div className="mt-3 space-y-2 text-sm text-white/55">{candidate.competencies.length > 0 && <p><span className="text-white/30">Компетенции:</span> {candidate.competencies.join(", ")}</p>}{Boolean(candidate.needs?.length) && <p><span className="text-white/30">Ищет помощь:</span> {candidate.needs?.join(", ")}</p>}{Boolean(candidate.industries?.length) && <p><span className="text-white/30">Отрасли:</span> {candidate.industries?.join(", ")}</p>}{Boolean(candidate.goals?.length) && <p><span className="text-white/30">Цели:</span> {candidate.goals?.join(", ")}</p>}<div className="flex flex-wrap gap-3 text-xs"><a href={`mailto:${candidate.email}`} className="text-blue-300">{candidate.email}</a>{candidate.telegram && <a href={`https://t.me/${candidate.telegram.replace(/^@/, "")}`} target="_blank" rel="noreferrer" className="text-blue-300">{candidate.telegram}</a>}</div></div></details><textarea value={invitationMessages[candidate.membership_id] || ""} onChange={(event) => setInvitationMessages((current) => ({ ...current, [candidate.membership_id]: event.target.value }))} rows={2} maxLength={1000} placeholder="Сообщение участнику" className="workspace-input mt-4 resize-y" /><button type="button" onClick={() => void inviteCandidate(candidate)} disabled={Boolean(busy) || teamIsFull || invitedMembershipIds.has(candidate.membership_id)} className="workspace-button mt-3"><Send size={14} />{invitedMembershipIds.has(candidate.membership_id) ? "Приглашение отправлено" : "Пригласить в проект"}</button></article>)}{!pool.candidates.length && <p className="text-sm text-white/35">Свободных участников пока нет.</p>}</div></section>}
...
function Mode({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) { return <button type="button" onClick={onClick} className={`shrink-0 border-b-2 px-5 py-3 text-sm ${active ? "border-white text-white" : "border-transparent text-white/45"}`}>{children}</button>; }
