...
class ChatOrchestrator:
    ...
    async def _stream_chat(self, user_message: str, history: list, state: dict, active_node_id: str | None, rag_context: str = ""):
        ...
        system_prompt += (
            f"\n\nОТВЕЧАЙ СТРОГО НА РУССКОМ ЯЗЫКЕ. Использование китайских иероглифов или любых других языков КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО.\n"
            f"\n[ТЕКУЩАЯ ДАТА — ФАКТ ИЗ СИСТЕМЫ]\n"
            f"Сегодня {current_date} года. Текущий год — {current_year}. Это абсолютная истина, заданная системой, она не требует проверки и не зависит от контекста.\n"
            f"На прямые вопросы о дате или годе («какой сейчас год», «какое число») отвечай ПРЯМО и КОРОТКО: «Сегодня {current_date} года» — БЕЗ оговорок «судя по данным», «по контексту», «согласно реестру», «исходя из ваших ссылок». Ты знаешь дату напрямую, она не выводится из источников.\n"
            f"Документы из контекста, описывающие {current_year} год в будущем времени (прогнозы, планы), относятся к ИСТОРИЧЕСКИМ данным — игнорируй их «прогнозную» рамку и используй цифры как фактические.\n"
            f"\n[ИСТОЧНИКИ ДЛЯ ОТВЕТА]\n"
            f"При вопросах о статистике, рыночных цифрах или конкретных фактах {current_year} года опирайся на данные веб-поиска и RAG-контекст. Если их нет — честно скажи об этом, а не выдумывай.\n"
            f"\n[ФОРМАТ ОТВЕТА]\n"
            f"Перед генерацией ответа напиши свои рассуждения внутри тегов <think>...</think>, затем дай итоговый ответ. "
            f"Каждый этап анализа (поиск, расчёт, синтез) начинай с тега [STATUS: Название этапа] (СТРОГО САМЫМ ПЕРВЫМ ТОКЕНОМ).\n"
            f"Внутри <think> рассуждай ТОЛЬКО по существу задачи — какие данные есть, какие выводы из них, какие гипотезы. "
            f"НЕ пересказывай мета-инструкции про язык, формат, Markdown, иероглифы, длину ответа — они применяются автоматически. "
            f"Фразы вида «Мне нужно ответить на русском», «структурирую в Markdown», «без иероглифов» — запрещены: пользователь видит их и они выглядят как утечка системного промпта."
        )
    ...
    async def _parse_thought_generator(self, generator):
        ...
        buffer = ""
        tags = [("<thought>", "</thought>"), ("<think>", "</think>"), ("<tool_call>", "</tool_call>"), ("<tool_thought>", "</tool_thought>"), ("<think_process>", "</think_process>"), ("ǏǏǏ", "ǏǏǏ"), ("[STATUS:", "]")]
    ...
    async def process_message(self, user_message: str, active_node_id: str = None, client_id: str = None, assistant_client_id: str = None, use_deep_search: bool = False, use_research: bool = False):
        ...
        # yield self._format_sse({"type": "status", "content": "Анализирую профиль стартапа и интент..."}) # Already yielded above
