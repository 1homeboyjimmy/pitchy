...
export function ResidentMembershipView({ membership, quotas, onChanged }: { membership: ResidentMembership; quotas: Record<string, ResidentQuota>; onChanged: () => Promise<void> }) {
  ...
  const navigate = (next: ResidentSection, targetId?: number) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const target = (section === "program" ? document.getElementById("resident-program-material-" + focusTarget) : null) || document.getElementById("resident-" + section + "-" + focusTarget);
      if (target) {
  ...
  return (
    <div className="min-w-0 lg:grid lg:grid-cols-[236px_minmax(0,1fr)]" data-testid="resident-membership-workspace">
      ...
      <div className="min-w-0 space-y-6 px-4 py-5 sm:px-6 lg:px-10 lg:py-8">
        <section data-testid="resident-membership-header" className="flex flex-wrap items-center justify-between gap-2 text-sm text-white/55">
          <span>{membership.accelerator.name} · {membership.cohort.name}</span>
          <span>{enrolled ? "Участник" : completed ? "Участие завершено" : membership.status === "accepted" ? "Принят" : membership.status}</span>
        </section>
        {membership.status === "accepted" && <section className="workspace-card"><h2 className="text-xl">Вас приняли в программу</h2><p className="mt-2 text-sm text-white/60">Подтвердите участие, чтобы открыть программу.</p><button type="button" onClick={() => void join()} disabled={joining} className="workspace-button mt-4">{joining && <Loader2 size={15} className="animate-spin" />}Подтвердить участие</button>{joinError && <p role="alert" className="mt-3 text-red-200">{joinError}</p>}</section>}
        {membership.status === "suspended" && <section className="workspace-card"><h2>Участие приостановлено</h2><p className="mt-2 text-sm text-white/60">Для восстановления доступа обратитесь к организатору потока.</p></section>}
        {membership.status === "withdrawn" && <section className="workspace-card"><h2>Участие завершено</h2><p className="mt-2 text-sm text-white/60">Рабочие разделы этого потока больше недоступны.</p></section>}
        {completed && <section className="workspace-card"><h2>Программа завершена</h2><p className="mt-2 text-sm text-white/60">Итоговый снимок результатов сохранён.</p></section>}

      {enrolled && section === "today" && <ResidentToday membership={membership} onNavigate={navigate} />}
      ...
      {enrolled && section === "tools" && <ResidentProjectRecommendations membershipId={membership.membership_id} onNavigate={(next) => navigate(next as ResidentSection)} />}
...
function ResidentSubTab({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} className={`block w-full rounded-lg px-3 py-2 text-left text-sm transition ${active ? "border-white/30 bg-white/10 text-white" : "border-white/8 text-white/40 hover:text-white/70"}`}>{children}</button>;
}
...
function ResidentEvents({ membershipId }: { membershipId: number }) {
  ...
  return <section className="workspace-card"><h2 className="text-xl">Мероприятия</h2><p className="mt-1 text-sm text-white/40">Время показано в часовом поясе потока. Для посещения используйте QR-код организатора.</p>{error && <p role="alert" className="mt-4 text-sm text-red-200">{error}</p>}{loading ? <Loader2 className="mx-auto mt-5 animate-spin text-white/40" /> : !events.length ? <p className="py-5 text-sm text-white/35">Опубликованных мероприятий пока нет.</p> : <div className="mt-5 grid gap-3 sm:grid-cols-2">{events.map((event) => <article id={"resident-events-" + event.id} tabIndex={-1} key={event.id} className={`rounded-2xl border p-4 ${event.status === "cancelled" ? "border-red-300/15 bg-red-300/[0.025]" : "border-white/9 bg-white/[0.02]"}`}>{event.preview_url && <Image unoptimized src={event.preview_url} alt={`Обложка события ${event.title}`} width={720} height={280} className="mb-4 h-36 w-full rounded-xl object-cover" />}<div className="flex items-start justify-between gap-3"><CalendarDays size={18} className="text-white/40" /><div className="flex flex-wrap justify-end gap-2">{event.status !== "published" && <span className={`rounded-full px-2 py-1 text-xs ${event.status === "completed" ? "bg-emerald-400/10 text-emerald-300" : "bg-red-400/10 text-red-200"}`}>{event.status === "completed" ? "Завершено" : "Отменено"}</span>}{event.attendance && <span className="rounded-full bg-emerald-400/10 px-2 py-1 text-xs text-emerald-300">{event.attendance.status === "present" ? "Посещение отмечено" : event.attendance.status}</span>}</div></div><h3 className="mt-4">{event.title}</h3><p className="mt-2 text-sm text-white/45">{new Date(event.starts_at).toLocaleString("ru-RU", { timeZone: event.timezone || undefined })} — {new Date(event.ends_at).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", timeZone: event.timezone || undefined })}{event.timezone ? ` · ${event.timezone}` : ""}</p><p className="mt-1 text-xs text-white/35">{event.event_format === "online" ? "Онлайн" : event.event_format === "offline" ? "Очно" : "Гибрид"}{event.online_platform ? ` · ${event.online_platform}` : ""}{event.host_name ? ` · ведущий ${event.host_name}` : ""}</p>{event.description && <p className="mt-3 whitespace-pre-wrap text-sm text-white/45">{event.description}</p>}{event.location && <p className="mt-2 text-sm text-white/40"><MapPin size={13} className="mr-1 inline" />{event.location}</p>}{event.venue_details && <p className="mt-1 text-xs text-white/35">{event.venue_details}</p>}<div className="mt-3 flex flex-wrap gap-3">{event.meeting_url && event.status !== "cancelled" && <a href={event.meeting_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm text-blue-300 underline">Подключиться <ExternalLink size={13} /></a>}{event.map_url && <a href={event.map_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm text-blue-300 underline">Карта <ExternalLink size={13} /></a>}{event.recording_url && <a href={event.recording_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm text-blue-300 underline">Запись <ExternalLink size={13} /></a>}</div>{event.cancellation_reason && <p className="mt-3 text-sm text-red-200">Причина отмены: {event.cancellation_reason}</p>}{!!event.homework_links.length && <div className="mt-4 flex flex-wrap gap-2">{event.homework_links.map((link) => <span key={link.assignment_id} className="rounded-full border border-white/8 px-3 py-1.5 text-xs text-white/45">ДЗ {link.relation === "before" ? "до" : link.relation === "during" ? "во время" : "после"}: {link.title || `#${link.assignment_id}`}</span>)}</div>}{event.outcome && <p className="mt-4 text-sm text-white/55"><span className="text-white/35">Итог:</span> {event.outcome}</p>}{event.next_step && <p className="mt-2 text-sm text-white/55"><span className="text-white/35">Следующий шаг:</span> {event.next_step}</p>}{!!event.post_materials.length && <div className="mt-3 flex flex-wrap gap-2">{event.post_materials.map((item) => <a key={item.url} href={item.url} target="_blank" rel="noreferrer" className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-blue-200">{item.title}</a>)}</div>}</article>)}</div>}</section>;
}
