...
const MODULES = [
  ...
  { key: "pitchy_artifacts", label: "Результаты Pitchy", text: "Действия программы, готовые результаты и управляемый доступ" },
] as const;
...
export function AcceleratorSetupWizard({ token, onCreated, onCancel }: { token: string; onCreated: (result: SetupResult) => Promise<void> | void; onCancel?: () => void }) {
  ...
  const [modules, setModules] = useState({ homework: true, attendance: true, progress_tracking: true, matchmaking: true, project_audit: true, demo_day: true, pitchy_artifacts: true });
