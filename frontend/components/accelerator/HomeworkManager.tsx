...
export function HomeworkManager({ cohortId, token, residents, isAdmin, pitchyEnabled, focusId }: { cohortId: number; token: string; residents: Resident[]; isAdmin: boolean; pitchyEnabled: boolean; focusId?: number }) {
  ...
  return (
    <section className="workspace-card">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl">Домашние задания</h2><p className="mt-1 text-sm text-white/40">Создавайте задания и задавайте аудиторию.</p>{isAdmin && <p className="mt-1 text-xs text-white/30">Общая настройка Pitchy используется только как значение по умолчанию для новых заданий.</p>}</div><div className="flex flex-wrap gap-2">{isAdmin && <button type="button" onClick={() => void togglePitchy()} disabled={busy === "pitchy-feature"} className="workspace-button !bg-transparent !text-white"><Sparkles size={15} />Pitchy по умолчанию: {featureEnabled ? "включён" : "выключен"}</button>}<button onClick={() => { if (showForm) resetForm(); else { setForm(emptyForm(featureEnabled)); setShowForm(true); } }} className="workspace-button"><Plus size={15} />{showForm ? "Закрыть" : "Новое задание"}</button></div></div>
