...
async def create_research(payload: ResearchCreate, user: User = Depends(get_async_current_user), db: AsyncSession = Depends(get_async_db)):
    ...
    if not limits.can_use_research:
        raise HTTPException(status_code=403,detail="Полное исследование доступно на тарифах Starter и Pro.")
    session=(await db.execute(select(ChatSession).where(ChatSession.id==payload.session_id,ChatSession.user_id==user.id))).scalar_one_or_none()
    if not session: raise HTTPException(status_code=404,detail="Session not found")
    active=(await db.execute(select(ResearchJob).where(ResearchJob.user_id==user.id,ResearchJob.status.in_(["queued","running","cancelling"])))).scalars().first()
    if active: raise HTTPException(status_code=409,detail="Дождитесь завершения текущего исследования.")
